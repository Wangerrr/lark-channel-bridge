 ...
 *
 * 1. Humans: `lark-channel-bridge-wg1 secrets set/list/remove` to manage the
 *    encrypted keystore manually.
 *
 * 2. lark-cli (and any other tool implementing the exec-provider protocol):
 *    `lark-channel-bridge-wg1 secrets get` reads a JSON-RPC request
...
export async function runSecretsSet(
  ...
  if (!appId) {
    console.error('用法: lark-channel-bridge-wg1 secrets set --app-id <id>');
    process.exit(1);
  }
...
export async function runSecretsRemove(
  ...
  if (!appId) {
    console.error('用法: lark-channel-bridge-wg1 secrets remove --app-id <id>');
    process.exit(1);
  }
