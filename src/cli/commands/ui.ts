...
export async function runUi(opts: RunUiOptions = {}): Promise<void> {
  ...
  console.error(
    [
      ...
      '  lark-channel-bridge-wg1 run --web-ui        # 前台运行 supervisor + 控制台',
      '  lark-channel-bridge-wg1 start --web-ui      # 作为后台服务运行 supervisor + 控制台',
      '',
      '（不带 --web-ui 的 run/start 是单 profile 无界面运行，适合无浏览器环境。）',
      '',
      '启动后再次运行 `lark-channel-bridge-wg1 ui` 打开控制台。',
    ].join('\n'),
  );
  process.exitCode = 1;
}
