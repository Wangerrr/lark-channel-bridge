...
/**
 * Pretty-print the list of running lark-channel-bridge-wg1 processes.
 *
...
export async function runKillCli(target: string | undefined): Promise<void> {
  if (!target) {
    console.error('用法: lark-channel-bridge-wg1 kill <bot id 或序号>');
    process.exit(1);
  }
  const entry = resolveTarget(target);
  if (!entry) {
    console.error(`✗ 没找到匹配的 bot:${target}`);
    console.error('  用 `lark-channel-bridge-wg1 ps` 看可选目标。');
    process.exit(1);
  }
...
function findOwningService(
  ...
  for (const { serviceId, flag } of candidates) {
    ...
    return {
      ...
      stopHint: `lark-channel-bridge-wg1 stop ${flag}`,
      restartHint: `lark-channel-bridge-wg1 restart ${flag}`,
    };
  }
  return undefined;
}
